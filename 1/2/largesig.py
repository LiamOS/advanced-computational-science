...
def run(sig,dx,Time,Dist):
    ...
    for j in range(0,nx):
        x[j] = Dist[0] + j*dx
    # Initialise the t_0 part of the array; Gaussian with sigma=sig and mu=20.
    for j in range(1,nx-1):
        U[0][j] = e**(-((x[j]-20)**2)/(2*(sig**2)))
    ...
    for j in range(1,nx-1):
        U[1][j] = U[0][j] + dt*(-((x[j]-20)/(sig**2))*(e**(-((x[j]-20)**2)/(2*(sig**2)))))
    ...
    # No need to worry about t=0,tmax boundaries
    # Now start compuating H generally.
    ...
    Tmax = 40.*nt/(Time[1] - Time[0])
    clf()
    plot(x,U[Tmax],'-')
    show()
    jmid = int(2*nx/3)
    T = dx*sum(H[Tmax][0:jmid])
    #R += dx*sum(H[Tmax][jmid:]
    E = dx*sum(H[Tmax])
    return T/E
...
sigmas = []
Ts = []
for s in [5]: #linspace(3,0.5,20):
     sigmas.append(s)
     Ts.append(run(s,0.1,aTime,aDist))
