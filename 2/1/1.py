#!/usr/bin/python3
# As a starting comment, making 3d plots in Python looks a bit nasty on the code-side.
# You'll see a fair lot of
#     fig = figure()
#     ax = fig.add_subplot(111, projection='3d')
#     ax.plot_surface(lX,lY,U[0],cmap=cm.coolwarm)
# in the code, which is not as pretty as mesh() in matlab or whatever, but it does the job.
...
xlabel("x")
ylabel("y")
savefig("t0.00000.pdf",format='PDF')
show()
...
for n in range(1,nt):
    ...
    if ((n*dt+0.01) % 0.01 == 0):
        ...
        title("t = "+str(n*dt))
        savefig("t"+str(round(n*dt,5))+".pdf",format='PDF')
        #show()
